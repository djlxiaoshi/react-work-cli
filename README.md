...
按照如上操作即可,然后再添加`src/**/*.css`在`.gitignore`文件中。接下来我们希望项目能够监测scss的变化，从而自动去编译scss文件，显然我们要用到`watch-css`和`build-css`这两个任务，我们可以使用`&&`操作符，但是这种跨平台性并不是很好，所以我们采用`npm-run-all`这个工具。
